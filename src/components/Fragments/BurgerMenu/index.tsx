...
export default function MainMenu<T extends MainMenuProps>({
  initialCookies,
}: T) {
  const [cookies, setCookies] = useState(initialCookies);
  const [store, setStore] = useState<Store[]>([]);

  const id = cookies?.map((user) => user?.id).toString();
  const storeValidation = store.find((userId) => userId.user_id === Number(id));
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/stores/", {
        method: "POST",
        body: JSON.stringify({ name: StoreName, user_id: Number(id) }),
      });
      if (res.ok) {
        alert("Success");
        return res;
